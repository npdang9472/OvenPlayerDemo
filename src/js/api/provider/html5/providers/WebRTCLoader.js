...
const WebRTCLoader = function (provider,
    ...
    function createClientPeerConnection(hostId, clientId, retryCount = 0) {
        const maxRetries = 3;
        ...
        if (!mainStream) {
            ...
            setTimeout(function () {
                ...
                createClientPeerConnection(hostId, clientId, retryCount);
            }, 100);

            return;
        }

        // Additional validation for media stream
        if (!mainStream.active) {
            OvenPlayerConsole.error("Main stream is not active, cannot create peer connection");
            let tempError = ERRORS.codes[504]; // WebRTC peer createOffer failed
            tempError.error = new Error("Media stream is not active");
            internalErrorCallback(tempError);
            return;
        }

        if (mainStream.getTracks().length === 0) {
            OvenPlayerConsole.error("Main stream has no tracks, cannot create peer connection");
            let tempError = ERRORS.codes[504]; // WebRTC peer createOffer failed
            tempError.error = new Error("Media stream has no tracks");
            internalErrorCallback(tempError);
            return;
        }
        ...
        // Debug logging before createOffer
        OvenPlayerConsole.log("Creating offer with:", {
            peerId: clientId,
            hostId: hostId,
            ...
            mainStreamActive: mainStream ? mainStream.active : 'no_stream',
            mainStreamTracks: mainStream ? mainStream.getTracks().length : 0,
            audioTracks: mainStream ? mainStream.getAudioTracks().length : 0,
            videoTracks: mainStream ? mainStream.getVideoTracks().length : 0
        });
        ...
        function handleCreateOfferError(event) {
            OvenPlayerConsole.error("WebRTC createOffer failed (attempt " + (retryCount + 1) + "/" + (maxRetries + 1) + "):", event);

            // Log detailed error information
            console.error("CreateOffer Error Details:", {
                error: event,
                peerId: clientId,
                hostId: hostId,
            ...
            if (retryCount < maxRetries) {
                OvenPlayerConsole.log("Retrying createOffer in " + retryDelay + "ms (attempt " + (retryCount + 2) + "/" + (maxRetries + 1) + ")");
                setTimeout(function () {
                    createClientPeerConnection(hostId, clientId, retryCount + 1);
                }, retryDelay);
                return;
            }

            // All retries exhausted, trigger error callback
            let tempError = ERRORS.codes[504]; // WebRTC peer createOffer failed
            tempError.error = event;
            tempError.reason = "CreateOffer failed after " + (maxRetries + 1) + " attempts";
